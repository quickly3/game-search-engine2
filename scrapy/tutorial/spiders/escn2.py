...
class AliSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        try:
            esDaily = Session.query(EsDaily).filter(
                EsDaily.state == 'init').order_by(EsDaily.id).first()
    ...
    def parse(self, response):
        ...
        for content in contents:
            ...
            if match is not None:
                ...
                duplicate_record = Session.query(EsDailyItem).filter(
                    EsDailyItem.link == link).first()

                if duplicate_record == None:
                    daily_item_obj = EsDailyItem(
                        pid=self.current_id, title=content, link=link, state="init", ver=ver)
        ...
        Session.query(EsDaily).filter(
            EsDaily.id == self.current_id).update(update_obj)
        Session.commit()

        try:
            next_item = Session.query(EsDaily).filter(
                EsDaily.state == 'init').order_by(EsDaily.id).first()
