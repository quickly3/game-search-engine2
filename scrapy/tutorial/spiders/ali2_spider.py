...
class Ali2Spider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        game = Session.query(Game).filter(
            text("state=0")).order_by(Game.id).first()
    ...
    def parse(self, response):
        ...
        id_zone_url = response.css(
            '#ali_comment_pc_global::attr(src)').extract_first()
        ...
        if id_zone_url != None:
            ...
            id_zone_url = response.css(
                '#ali_hits_pc_global::attr(src)').extract_first()
        ...
        name_chs = response.css(
            '.detail_game_l_r_ctit h1::text').extract_first()
        name_chs = dr.sub('', name_chs)

        inLanguage = response.css(
            '.detail_game_l_r_ctit span[itemprop=inLanguage]::text').extract_first()
        license = response.css(
            '.detail_game_l_r_ctit span[itemprop=license]::text').extract_first()
        name_en = response.css(
            '.detail_game_l_r_ctit span[itemprop=alias]::text').extract_first()

        game_type = response.css('.detail_game_l_r_info ul li')[
            0].css("a::text").extract_first()

        publish_date_by_ali = response.css('.detail_game_l_r_info ul li')[
            1].css("a span::text").extract_first()

        publisher = response.css('.detail_game_l_r_info ul li')[
        ...
        game_images_mini = response.css(
            '#smallImg span img::attr(src)').extract()
        if len(game_images_mini) > 0:
            game_images_mini.pop()
            game_images_mini_string = ",".join(game_images_mini)

        game_images_string = ""
        game_images = response.css(
            '.detail_body_con_bb_con_con .detail_body_con_jt_con_img img::attr(src)').extract()
        ...
        description_arr = response.css(
            '#yxjs .detail_body_left_info_con p').extract()
        ...
        gameSetting = response.css(
            '#pzyq .detail_body_left_info_con .gameSetting ul')

        if len(gameSetting) > 0:

            sys_requirements_key = gameSetting[0].css(
                'li span::text').extract()
            sys_requirements_value1 = gameSetting[1].css(
                'li span::text').extract()
            sys_requirements_value2 = gameSetting[2].css(
                'li span::text').extract()
        ...
        install_info = response.css(
            '#azsm .detail_body_left_info_con p::text').extract()
        ...
        Session.query(Game).filter(
            Game.id == self.current_id).update(update_obj)
        Session.commit()
