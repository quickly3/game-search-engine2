    ...
    def parse(self, response):
        ...
        for item in items:
            ...
            if int(item['order']['pub_date']) > 0:
                item['order']['pub_date'] = datetime.datetime.fromtimestamp(
                    item['order']['pub_date'])
            else:
                del item['order']['pub_date']

            if int(item['order']['pub_real_time']) > 0:
                item['order']['pub_real_time'] = datetime.datetime.fromtimestamp(
                    item['order']['pub_real_time'])
            else:
                del item['order']['pub_real_time']

            if 'renewal_time' in item['order']:
                item['order']['renewal_time'] = datetime.datetime.fromtimestamp(
                    item['order']['renewal_time'])

            item['order']['follow_string'] = item['order']['follow']
            item['order']['play_string'] = item['order']['play']

            item['order']['follow'] = item['order']['follow'].replace(
                "人追番", "")
            item['order']['follow'] = item['order']['follow'].replace("追番", "")
            item['order']['follow'] = item['order']['follow'].replace(
                "人想看", "")
            ...
            if "万" in item['order']['follow']:
                item['order']['follow'] = item['order']['follow'].replace(
                    "万", "")
                item['order']['follow'] = float(
                    item['order']['follow']) * 10000
                follow_dealed = True
            ...
            if not play_dealed and "亿" in item['order']['play']:
                ...
                item['order']['play'] = float(
                    item['order']['play']) * 100000000
                play_dealed = True
            ...
            item['ssid'] = int(item['link'].replace(
