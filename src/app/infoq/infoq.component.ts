...
export class InfoqComponent {
  ...
  @ViewChild('categoriesTh', { static: true }) categoriesTh:
    | NgbTypeahead
    | undefined;
  ...
  searchTags = (text$: Observable<string>) => {
    ...
    const clicksWithClosedPopup$ = this.tagsClick$.pipe(
      filter(() => {
  ...
  removeSelectTag = (tag) => {
    if (this.queryParams.selectTags.indexOf(tag) > -1) {
      this.queryParams.selectTags = this.queryParams.selectTags.filter(
        (item) => item !== tag
      );
      this.search();
    }
  ...
  searchCategories = (text$: Observable<string>) => {
    ...
    const clicksWithClosedPopup$ = this.categoriesClick$.pipe(
      filter(() => {
  ...
  removeSelectCategory = (category) => {
    if (this.queryParams.selectCategories.indexOf(category) > -1) {
      this.queryParams.selectCategories =
        this.queryParams.selectCategories.filter((item) => item !== category);
      this.search();
    }
  };
  ...
  updateQueryParamsByUrl(): void {
    ...
    if (urlParamsCopy.source) {
      urlParamsCopy.source = this.sourceList.find(
        (i) => i.title === urlParamsCopy.source
      );
    }
    ...
    if (urlParamsCopy.selectCategories) {
      urlParamsCopy.selectCategories =
        urlParamsCopy.selectCategories.split(',');
    }

    if (urlParamsCopy.sortBy) {
      urlParamsCopy.sortBy = this.sortItems.find(
        (i) => i.value === urlParamsCopy.sortBy
      );
    }

    if (urlParamsCopy.subNavModel) {
      this.subNavModel = urlParamsCopy.subNavModel;
      this.curSubNav = this.subNavItems.find(
        (i) => i.name === urlParamsCopy.subNavModel
      );
    }
    ...
    if (e.key === 'Enter') {
      if (this.instance) {
        ...
        if (
          value &&
          value.trim() !== '' &&
          this.queryParams.selectTags.indexOf(value) < 0
        ) {
          this.queryParams.selectTags.push(value);
          this.tagInput = '';
          this.search();
    ...
    if (e.key === 'Enter') {
      if (this.instance) {
        ...
        if (
          value &&
          value.trim() !== '' &&
          this.queryParams.selectCategories.indexOf(value) < 0
        ) {
          this.queryParams.selectCategories.push(value);
          this.categoryInput = '';
          this.search();
  ...
  getTags = () => {
    this.InfoqService.getTags({
      ...
      const _tags: any[] = tags.map((i: { key: any; doc_count: number }) => {
        const matchedItem = this.tagsI18n.find((item: { text: any }) => {
  ...
  getCategories = () => {
    this.InfoqService.getCategories({
      ...
      let total = 0;
      // tslint:disable-next-line: variable-name
      const _cates: any[] = cates.map((i: { key: any; doc_count: number }) => {
        total += i.doc_count;
        return {
          text: i.key,
          count: i.doc_count,
  ...
  setQueryParamsToUrl(queryStringParams): void {
    ...
    for (const param in queryStringParams) {
      if (
        typeof queryStringParams[param] !== 'string' ||
        queryStringParams[param].trim() !== ''
      ) {
        switch (param) {
          case 'updateSta':
            break;
  ...
  search = (option = { updateSta: true }) => {
    ...
    if (this.curSubNav.name === 'tags') {
      this.searchTagsAggs(option);
    }

    if (this.curSubNav.name === 'cates') {
      this.searchCatesAggs(option);
    }
    ...
    if (this.curSubNav.name === 'cloud') {
      ...
      if (
        ...
        this.selectCloudWords = this.queryParams.keywords
          .replace(/ {2,}/g, ' ')
          .trim()
          .split(' ');
      }
  ...
  searchAuthorAggs = (option = { updateSta: true }) => {
    ...
    this.InfoqService.getAuthorTermsAgg(queryStringParams).subscribe(
      (resp: any) => {
        this.authorAggs = resp.data.map((d) => ({
          name: d.key,
          value: d.doc_count,
        }));
      }
  ...
  searchTagsAggs = (option = { updateSta: true }) => {
    const queryStringParams = this.getQueryParams(option);
    this.setQueryParamsToUrl(queryStringParams);
    this.InfoqService.getTagsTermsAgg(queryStringParams).subscribe(
      (resp: any) => {
        this.tagsAggs = resp.data.map((d) => ({
          name: d.key,
          value: d.doc_count,
        }));
      }
  ...
  searchCatesAggs = (option = { updateSta: true }) => {
    const queryStringParams = this.getQueryParams(option);
    this.setQueryParamsToUrl(queryStringParams);
    this.InfoqService.getCatesTermsAgg(queryStringParams).subscribe(
      (resp: any) => {
        this.catesAggs = resp.data.map((d) => ({
          name: d.key,
          value: d.doc_count,
        }));
      }
  ...
  searchDatasSimple = (term: any) => {
    ...
    return this.InfoqService.searchDatasSimple(params).pipe(
      map((response: any) => {
        const titles = response.data.map((item: { title: any }) => item.title);
        return titles;
      })
    );
  };
  ...
  clickHorizontalBarForTags = (i): any => {
    if (this.tagsAggs[i]) {
      this.queryParams.selectTags = [this.tagsAggs[i].name];
      this.subNavChange(this.subNavItems[0]);
      this.search();
    }
  };

  clickHorizontalBarForCates = (i): any => {
    if (this.catesAggs[i]) {
      this.queryParams.selectCategories = [this.catesAggs[i].name];
      this.subNavChange(this.subNavItems[0]);
      this.search();
    }
  };
  ...
  removeSelectCloudWord = (txt) => {
    this.queryParams.keywords = this.queryParams.keywords
      .replace(txt, '')
      .trim();
    this.search();
  };
